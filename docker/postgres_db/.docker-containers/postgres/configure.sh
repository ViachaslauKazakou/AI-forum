...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    ...
    SELECT 'CREATE DATABASE $POSTGRES_DB_NAME' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '$POSTGRES_DB_NAME')\gexec
...
psql --username "$POSTGRES_USER" -d "$POSTGRES_DB_NAME" -c "CREATE SCHEMA IF NOT EXISTS $POSTGRES_DB_SCHEMA AUTHORIZATION $POSTGRES_DB_USER"
